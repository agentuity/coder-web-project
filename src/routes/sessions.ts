...
api.post("/", async (c) => {
  ...
  const body: CreateSessionBody = await c.req
      ...
      async (parentSpan) => {
        ...
        try {
          ...
          try {
            ...
            logger.warn("Failed to load GitHub token for sandbox", {
              userId: user.id,
            });
          }
          ...
          if (snapshotRecord) {
            ...
            const snapMeta = (snapshotRecord.metadata ?? {}) as Record<
              string,
              unknown
            >;
            const workDir =
              typeof snapMeta.workDir === "string"
                ? snapMeta.workDir
          ...
          for (let attempt = 1; attempt <= 5; attempt++) {
            try {
              ...
              logger.info(`session.create attempt ${attempt} response`, {
                ...
                keys:
                  opencodeSession && typeof opencodeSession === "object"
                    ? Object.keys(opencodeSession)
                    : [],
              ...
              logger.warn(
                `session.create attempt ${attempt}: no session ID returned`,
                {
                  response: JSON.stringify(opencodeSession).slice(0, 500),
                },
              );
            } catch (err) {
              logger.warn(`session.create attempt ${attempt} failed`, {
                error: String(err),
              });
            }
          ...
          if (body.prompt && opencodeSessionId) {
            try {
              const commandSlug = body.agent
                ? body.agent.replace(/^\//, "")
                : null;

              // Parse model into providerID/modelID (same as chat.ts)
              ...
              // Non-template commands (agentuity-coder) can use the agent field.
              if (commandSlug && TEMPLATE_COMMANDS.has(commandSlug)) {
                // Send as slash command: "/<command> <prompt>" — OpenCode expands the template
                await client.session.promptAsync({
                  path: { id: opencodeSessionId },
                  body: {
                    parts: [
                      { type: "text", text: `/${commandSlug} ${body.prompt}` },
                    ],
                    ...(providerID && modelID
          ...
          const isConnectionError =
            /websocket|ECONNREFUSED|ECONNRESET|ETIMEDOUT|fetch failed|network/i.test(
              errorMsg,
            );
          const isAuthError = /unauthorized|401|403|api.key|auth/i.test(
            errorMsg,
          );
          let hint = "";
          if (isAuthError) {
            hint =
              ' — This may indicate missing API keys. Ensure ANTHROPIC_API_KEY and OPENAI_API_KEY org secrets are configured via "agentuity cloud env set".';
          } else if (isConnectionError) {
            hint =
              " — Sandbox connection failed. This may indicate the OpenCode server did not start. Check that org secrets (ANTHROPIC_API_KEY, OPENAI_API_KEY) are set and the sandbox has network access.";
          }
  ...
  const updatedSessions = await Promise.all(
    result.map(async (session) => {
      ...
      if (typeof meta.opencodePassword === "string") {
        try {
          healthAuthHeader = buildBasicAuthHeader(
            decrypt(meta.opencodePassword),
          );
        } catch {
          // Decryption failed
        }
